...
export const LoadingToast = ({ text = "Validating..." }) => {
  return (
    <div
      className="flex items-center w-full max-w-xs p-4 space-x-4 divide-gray-200 rounded-lg "
      role="alert"
    >
      <svg
      ...
      <div className="pl-4 text-lg font-semibold text-orange-500">{text}</div>
    </div>
  );
};

export const ErrorToast = ({ text = "Error occured" }) => {
  return (
    <div
      className="flex items-center w-full max-w-xs p-4 space-x-4 divide-gray-200 rounded-lg "
      role="alert"
    >
      <BiErrorAlt className="inline w-8 h-8 mr-2 text-gray-200 dark:text-gray-600 fill-red-700" />
