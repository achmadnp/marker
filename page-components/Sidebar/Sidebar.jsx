...
const Sidebar = ({ withButton = true }) => {
  ...
  return (
    <div className="flex flex-nowrap">
      ...
      >
        {withButton && (
          <HiChevronDoubleLeft
            className={`absolute cursor-pointer backdrop-blur-md duration-1000 -right-3 top-9 w-7 h-7 border-dark-purple
        border-2 rounded-full  ${
          !open && "rotate-180 border-green-600 text-green-600"
        ...
        <ul className="pt-6 tracking-wider bg-inherit">
          ...
          <li
            className={`flex rounded-md my-5 p-2 font-semibold border-red-200 hover:animate-pulse cursor-pointer duration-300 border ease-linear bg-red-400/20 hover:border-2 hover:border-red-600 text-gray-300 items-center gap-x-4 hover:bg-red-500/30
            mt-9 `}
            onClick={signOut}
          >
            <div>
      ...
      <div
        className={`${
          open ? "translate-x-0" : "-translate-x-[256px]"
        } bg-[url('/imgs/assets/sidebarbg.jpg')] w-64 shadow-lg shadow-[#fff] min-h-screen h-max sm:hidden fixed transition duration-500 z-50`}
      >
        <HiChevronDoubleLeft
          className={`absolute cursor-pointer backdrop-blur-md duration-1000  top-9 w-7 h-7 border-dark-purple
        border-2 rounded-full  ${
          !open && "rotate-180 border-green-600 text-green-600 -right-8"
        } ${open && "border-red-600 text-red-600 -right-3"}`}
          onClick={() => setOpen(!open)}
        />
        ...
        <ul className="pt-6 mx-3 tracking-wider bg-inherit">
          {menus.map((menu, i) => (
            <li
              key={i}
              className={`flex backdrop-blur-sm rounded-md my-5 p-2 font-semibold  cursor-pointer duration-300 border ease-linear bg-slate-200/5 hover:border-2 hover:border-blue-300 text-gray-300 items-center gap-x-4 hover:bg-blue-400/30
    ...
    //     className={`${
    //       show
    //         ? "rotate-180 left-[1.3rem] translate-x-"
    //         : "rotate-0 -right-[2.5rem]"
    //     } duration-300 absolute top-12 z-10 border border-white rounded-full`}
    //   >
    ...
    //   <div
    //     className={`${
    //       show ? "w-full" : "-translate-x-full"
    //     } h-full px-8 bg-slate-800 ease-in duration-300`}
    //   >
    ...
    //     </div>
    //     <ul className="mt-12">
    //       <li className="items-center justify-between w-full mb-6 text-gray-300 cursor-pointer hover:text-gray-500">
  ...
  //     {show ? (
  //       <button
  //         className="fixed z-50 flex items-center text-4xl text-white cursor-pointer left-10 top-6"
  //         onClick={() => setShow(!show)}
  //       >
  //         x
  //       </button>
