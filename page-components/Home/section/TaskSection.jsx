...
export const TaskSections = ({ tasksa, max = 3 }) => {
  let tasks = tasksa || [
    {
      name: "testOrg1",
      memberCnt: 8,
  ...
  return (
    <div className="m-8">
      ...
      {tasks.length === 0 && <div>No Org registered</div>}
      {tasks.length > 0 && (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {tasks.slice(0, max).map((task, i) => (
            ...
            >
              <div className="relative flex flex-col justify-between w-full h-64 max-w-xl px-2 py-5 bg-gray-800 border border-gray-400 rounded-lg sm:px-4 sm:max-w-md ">
                <div className="z-10 h-full">
                  <h4 className="mb-3 text-lg font-bold text-center underline sm:text-start sm:no-underline text-fuchsia-300">
                    {task.name}
                  </h4>
                  <p className="text-gray-100">
                    Total member : {task.memberCnt}
                  </p>
                  <p className="text-gray-100">
                    Total Project : {task.projectCnt}
                  </p>
